...
func TestConnectionMethodDefaultsToInvite(t *testing.T) {
	c := ConnectionSection{User: "test-user", Key: "invite-key"}
...
func TestConnectionMethodSupportsPassword(t *testing.T) {
	c := ConnectionSection{User: "test-user", AuthMethod: AuthPassword, Password: "secret"}
...
func TestConnectionMethodInfersPasswordWhenOnlyPasswordIsSet(t *testing.T) {
	c := ConnectionSection{User: "test-user", Password: "secret"}
	if got := c.Method(); got != AuthPassword {
...
func TestConnectionHasCredentialsRequiresSelectedSecret(t *testing.T) {
	if (ConnectionSection{User: "test-user", AuthMethod: AuthPassword, Key: "invite-key"}).HasCredentials() {
		t.Fatalf("password auth should require password, not invite key")
	}
	if (ConnectionSection{User: "test-user", AuthMethod: AuthInvite, Password: "secret"}).HasCredentials() {
